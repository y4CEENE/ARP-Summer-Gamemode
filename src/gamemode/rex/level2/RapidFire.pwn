...
hook OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, Float:fX, Float:fY, Float:fZ)
{
    ...
    if ((22 <= weaponid <= 27) && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        if (delay < AntiHackMinimumFireDelay)
        {
            ...
            Log("logs/Rex/Anti-RapidFire.log", "Player: %s, Ping: %d, Suspect of: RapidFire warning %d/5, Delay: %s", GetPlayerNameEx(playerid), GetPlayerPing(playerid), PlayerFastFireWarnings[playerid], delay);
