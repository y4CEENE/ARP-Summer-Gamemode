...
static DealerID[MAX_PLAYERS];
static VehicleDealer[MAX_PLAYERS];
static InDealer[MAX_PLAYERS];
static DSMenuEnabled[MAX_PLAYERS];
static Float:DSAngle[MAX_PLAYERS];
static DealerShipCarIndex[MAX_PLAYERS];
static DealershipCars[MAX_DEALERSHIP_CARS][e_Dealership];
static DSListedItems[MAX_PLAYERS][100];
...
DB:OnDealershipCarAdded(id)
{
    DealershipCars[id][dcID] = GetDBInsertID();

    SaveDealershipCar(id);
}

DB:OnLoadDealershipCars()
{
    new
        rows = GetDBNumRows();
    ...
    for (new i = 0; i < rows; i ++)
    {
        ...
        DealershipCars[i][dcID] = GetDBIntField(i, "ID");
        DealershipCars[i][dcCompany] = GetDBIntField(i, "Company");
        DealershipCars[i][dcModel] = GetDBIntField(i, "Model");
        DealershipCars[i][dcPrice] = GetDBIntField(i, "Price");

        GetDBStringField(i, "maxspeed", DealershipCars[i][dcMaxSpeed], 12);
        GetDBStringField(i, "type", DealershipCars[i][dcType], 12);
        GetDBStringField(i, "doors", DealershipCars[i][dcDoors], 12);
...
DB:OnPlayerAttemptBuyVehicle(playerid, index)
{
    if (index < 0)
    {
        return;
    ...
    else
    {
        ...
        else
        {
            ...
            if (PlayerData[playerid][pDealership] == 1)
            {
                ...
                DBQuery("INSERT INTO vehicles (modelid, pos_x, pos_y, pos_z, pos_a, price, plate, color1, color2, gangid, respawndelay) VALUES(%i, '%f', '%f', '%f', '%f', %i, '%e', %i, %i, %i, %i)",
                    modelid, x, y, z, a, price, string, 1, 1, gangid, 600);
                DBFormat("SELECT * FROM vehicles WHERE id = LAST_INSERT_ID()");
                DBExecute("OnLoadVehicles");

                SendClientMessageEx(playerid, COLOR_GREEN, "Gang vehicle %s purchased for $%i. /gfindcar to locate the vehicle.", GetVehicleNameByModel(modelid), price);
                DBLog("log_property", "%s (uid: %i) purchased a %s for $%i for his gang %s.", GetPlayerNameEx(playerid), PlayerData[playerid][pID], GetVehicleNameByModel(modelid), price, GangInfo[gangid][gName]);

            }
            else
            {
                DBQuery("INSERT INTO vehicles (ownerid, owner, modelid, price, plate, pos_x, pos_y, pos_z, pos_a)"\
                        " VALUES(%i, '%e', %i, %i, '%e', '%f', '%f', '%f', '%f')",
                        PlayerData[playerid][pID], GetPlayerNameEx(playerid), modelid, price, string, x, y, z, a);

                SendClientMessageEx(playerid, COLOR_GREEN, "%s purchased for $%i. /carstorage to spawn this vehicle.", GetVehicleNameByModel(modelid), price);
                DBLog("log_property", "%s (uid: %i) purchased a %s for $%i.", GetPlayerNameEx(playerid), PlayerData[playerid][pID], GetVehicleNameByModel(modelid), price);
...
Dialog:DealerList(playerid, response, listitem, inputtext[])
{
    ...
    if (response)
    {
        ...
        else
        {
            ...
            PlayerData[playerid][pSelected] = DSListedItems[playerid][listitem - 3];
...
Dialog:DealerEditDSType(playerid, response, listitem, inputtext[])
{
    ...
    if (response)
    {
        ...
        DBQuery("UPDATE businesses SET dealershiptype = %i WHERE id = %i", BusinessInfo[company][bDealerShipType], BusinessInfo[company][bID]);
...
Dialog:DealerEdit(playerid, response, listitem, inputtext[])
{
    ...
    if (response)
    {
        ...
        switch (listitem)
        {
            ...
            case 1:
            {
                DBQuery("DELETE FROM rp_dealercars WHERE ID = %i", DealershipCars[vehicleid][dcID]);

                DealershipCars[vehicleid][dcExists] = 0;
                SendInfoMessage(playerid, "You have deleted a vehicle: %s.", GetVehicleNameByModel(DealershipCars[vehicleid][dcModel]));
...
Dialog:DealerType(playerid, response, listitem, inputtext[])
{
    if (response)
    {
        ...
        SendInfoMessage(playerid, "You have set the Type to '%s' for vehicle: %s.", DealershipCars[vehicleid][dcType], GetVehicleNameByModel(DealershipCars[vehicleid][dcModel]));
    }
...
Dialog:DealerMaxSpeed(playerid, response, listitem, inputtext[])
{
    if (response)
    {
        ...
        SaveDealershipCar(vehicleid);
        SendInfoMessage(playerid, "You have set the MaxSpeed to '%s' for vehicle: %s.", DealershipCars[vehicleid][dcMaxSpeed], GetVehicleNameByModel(DealershipCars[vehicleid][dcModel]));
    }
    ShowDealershipEditMenu(playerid, PlayerData[playerid][pCompany]);
    return 1;
}
...
Dialog:DealerDoors(playerid, response, listitem, inputtext[])
{
    if (response)
    {
        ...
        SaveDealershipCar(vehicleid);
        SendInfoMessage(playerid, "You have set the Doors to '%s' for vehicle: %s.", DealershipCars[vehicleid][dcDoors], GetVehicleNameByModel(DealershipCars[vehicleid][dcModel]));
    }
    ShowDealershipEditMenu(playerid, PlayerData[playerid][pCompany]);
    return 1;
}
...
Dialog:DealerPrice(playerid, response, listitem, inputtext[])
{
    ...
    if (response)
    {
        ...
        else
        {
            ...
            SendInfoMessage(playerid, "You have set the price to %s for vehicle: %s.", FormatCash(amount), GetVehicleNameByModel(DealershipCars[vehicle][dcModel]));
            ShowDealershipEditMenu(playerid, company);
        }
    }
    return 1;
...
hook OP_ClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
    ...
    if (playertextid == DSBuy)
    {
        ...
        calldb::OnPlayerAttemptBuyVehicle(playerid, DealerShipCarIndex[playerid]);
        CancelSelectTextDraw(playerid);
        HideDSTD(playerid);
        InDealer[playerid] = 0;
        DealerShipCarIndex[playerid] = -1;
...
ClearProducts(company)
{
    switch (BusinessInfo[company][bType])
    {
        case BUSINESS_DEALERSHIP:
        {
            ...
            DBQuery("DELETE FROM rp_dealercars WHERE Company = %i", BusinessInfo[company][bID]);
        }
    }
}
SaveDealershipCar(id)
{
    ...
    DBQuery("UPDATE rp_dealercars SET Model = %i, Price = %i, maxspeed='%e', type = '%e',doors='%e'  WHERE ID = %i", DealershipCars[id][dcModel], DealershipCars[id][dcPrice], DealershipCars[id][dcMaxSpeed], DealershipCars[id][dcType], DealershipCars[id][dcDoors],DealershipCars[id][dcID]);
    return 1;
}
...
AddVehicleToDealership(company, model, price)
{
    ...
    if (id != -1)
    {
        ...
        DBFormat("INSERT INTO rp_dealercars (Company) VALUES(%i)", DealershipCars[id][dcCompany]);
        DBExecute("OnDealershipCarAdded", "i", id);
    }
    return id;
}
...
Dialog:DealerAdd(playerid, response, listitem, inputtext[])
{
    ...
    if (response)
    {
        ...
        else
        {
            ...
            Dialog_Show(playerid, CarPrice, DIALOG_STYLE_INPUT, "{FFFFFF}Vehicle price", "Please input the price to set for '%s' below.", "Submit", "Cancel", GetVehicleNameByModel(modelid));
        }
    }
    return 1;
}
...
Dialog:CarPrice(playerid, response, listitem, inputtext[])
{
    ...
    if (response)
    {
        ...
        if (sscanf(inputtext, "i", amount))
        {
            return Dialog_Show(playerid, CarPrice, DIALOG_STYLE_INPUT, "{FFFFFF}Vehicle price", "Please input the price to set for '%s' below.", "Submit", "Cancel", GetVehicleNameByModel(modelid));
        }
        else if (amount < 1)
        {
            return Dialog_Show(playerid, CarPrice, DIALOG_STYLE_INPUT, "{FFFFFF}Vehicle price", "The price must be above $0.\n\nPlease input the price to set for '%s' below.", "Submit", "Cancel", GetVehicleNameByModel(modelid));
        }
        else
        {
            new
            ...
            {
                SendInfoMessage(playerid, "You have added a %s to company %i.", GetVehicleNameByModel(modelid), company);
                ShowDealershipEditMenu(playerid, company);
            }
        }
    }
...
ShowDealershipEditMenu(playerid, company)
{
    ...
    else
    {
        ...
        for (new i = 0; i < MAX_DEALERSHIP_CARS; i ++)
        {
            if (DealershipCars[i][dcExists] && DealershipCars[i][dcCompany] == BusinessInfo[company][bID])
            {
                format(string, sizeof(string), "%s\n%s (price: %s)", string, GetVehicleNameByModel(DealershipCars[i][dcModel]), FormatCash(DealershipCars[i][dcPrice]));
                DSListedItems[playerid][index++] = i;
            }
        }
...
CMD:editdscarprop(playerid,params[])
{
    ...
    if (!strcmp(param, "maxspeed", true))
    {
        ...
        DBQuery("UPDATE rp_dealercars SET maxspeed='%e' WHERE ID = %i", DealershipCars[vehid][dcMaxSpeed], DealershipCars[vehid][dcID]);
    ...
    else if (!strcmp(param, "type", true))
    {
        ...
        DBQuery("UPDATE rp_dealercars SET type = '%e' WHERE ID = %i", DealershipCars[vehid][dcType], DealershipCars[vehid][dcID]);
    ...
    else if (!strcmp(param, "doors", true))
    {
        ...
        DBQuery("UPDATE rp_dealercars SET doors='%e'  WHERE ID = %i", DealershipCars[vehid][dcDoors], DealershipCars[vehid][dcID]);
