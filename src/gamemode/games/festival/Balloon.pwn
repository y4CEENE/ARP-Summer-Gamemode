...
{
    BalloonTickets[playerid] = GetDBIntField(row, "balloon_tickets");
}
...
CMD:givetickets(playerid, params[])
{
    ...
    DBQuery("Update "#TABLE_USERS" set balloon_tickets = %i where uid = %i;", BalloonTickets[targetid], PlayerData[targetid][pID]);
...
Dialog:Balloon(playerid, response, listitem, inputtext[])
{
    ...
    DBQuery("Update "#TABLE_USERS" set balloon_tickets = %i where uid = %i;", BalloonTickets[playerid], PlayerData[playerid][pID]);
    RideAirBalloon(playerid);
    return 1;
}
