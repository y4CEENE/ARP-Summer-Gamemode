/// @file      EventSystem.pwn
/// @author    BOURAOUI Al-Moez L.A
/// @date      Created at 2021-04-25 23:00:47 +0100
...
static EventTop5[2][5][MAX_PLAYER_NAME + 8]; // Top five of each team
...
{
    if (!IsPlayerConnected(playerid) || !IsPlayerLoggedIn(playerid) || !JoinedEvent[playerid])
        return 1;

    if (!IsPlayerConnected(damagedid) || !IsPlayerLoggedIn(damagedid) || !JoinedEvent[damagedid])
        return 1;

    return Event[Event_TeamFire] == false && EventTeam[playerid] == EventTeam[damagedid] ? 0 : 1;
}
...
EventKickPlayer(playerid)
{
    ...
    switch (position)
    {
        case 1..5: format(EventTop5[EventTeam[playerid]][position - 1], MAX_PLAYER_NAME + 8, "[%d] %s", playerid, GetPlayerNameEx(playerid));
    }
    return 1;
}
...
PublishEvent(playerid)
{
    ...
    {
        case EventWhiteList_Faction:        format(msg, sizeof(msg), "* [Faction %s] New event created by %s /joinevent to join. *", GetEventTypeName(), GetAdminName(playerid));
        case EventWhiteList_GangAndFaction: format(msg, sizeof(msg), "* [Faction and gang %s] New event created by %s /joinevent to join. *", GetEventTypeName(), GetAdminName(playerid));
        case EventWhiteList_Gang:           format(msg, sizeof(msg), "* [Gang %s] New event created by %s /joinevent to join. *", GetEventTypeName(), GetAdminName(playerid));
        case EventWhiteList_VIP:            format(msg, sizeof(msg), "* [VIP %s] New event created by %s /joinevent to join. *", GetEventTypeName(), GetAdminName(playerid));
        default:                            format(msg, sizeof(msg), "* [%s] New event created by %s /joinevent to join. *", GetEventTypeName(), GetAdminName(playerid));
    }
