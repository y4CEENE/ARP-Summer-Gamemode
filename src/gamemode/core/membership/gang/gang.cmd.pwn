...
CMD:editgang(playerid, params[])
{
	...
	if(sscanf(params, "is[14]S()[128]", gangid, option, param))
	{
		...
		SendClientMessage(playerid, COLOR_SYNTAX, "List of options: Sign");
		return 1;
	}
	...
	else if(!strcmp(option, "sign", true))
    {
        new value;

        if(sscanf(param, "i", value))
        {
            return SendClientMessage(playerid, COLOR_SYNTAX, "USAGE: /editgang [gangid] [sign] [value]");
        }
        
        if(value < 0 || value > 10)
        {
            return SendClientMessage(playerid, COLOR_SYNTAX, "value must be beetwen 0 - 10");
        }
        
        GangInfo[gangid][gSign] = value;

        mysql_format(connectionID, queryBuffer, sizeof(queryBuffer), "UPDATE "#TABLE_GANGS" SET sign = %i WHERE id = %i", GangInfo[gangid][gSign], gangid);
        mysql_tquery(connectionID, queryBuffer);

        SendAdminMessage(COLOR_LIGHTRED, "AdmCmd: %s has set sign of gang ID %i to %i.", GetRPName(playerid), gangid, value);
    }
...
CMD:gsign(playerid, params[])
{
    ...
    if(GangInfo[gangid][gSign] == 0)
    {
        return SendClientMessage(playerid, COLOR_GREY, "Your gang dont have gang sign.");
    }
     
    new sign[10];
    format(sign, sizeof sign, "%d", GangInfo[gangid][gSign]);
    callcmd::zbibo9lwtrmtk(playerid, sign);
    return 1;
}
...
CMD:givemaxweapons(playerid, params[])
{
    ...
    GangInfo[gangid][gWeapons][GANGWEAPON_9MM] = 50;
    GangInfo[gangid][gWeapons][GANGWEAPON_SDPISTOL] = 50;
    GangInfo[gangid][gWeapons][GANGWEAPON_DEAGLE] = 50;
    GangInfo[gangid][gWeapons][GANGWEAPON_SHOTGUN] = 50;
    GangInfo[gangid][gWeapons][GANGWEAPON_TEC9] = 50;
    GangInfo[gangid][gWeapons][GANGWEAPON_UZI] = 50;
    GangInfo[gangid][gWeapons][GANGWEAPON_MP5] = 50;
    GangInfo[gangid][gWeapons][GANGWEAPON_AK47] = 50;
    GangInfo[gangid][gWeapons][GANGWEAPON_RIFLE] = 50;
    GangInfo[gangid][gWeapons][GANGWEAPON_M4] = 50;
    GangInfo[gangid][gWeapons][GANGWEAPON_SPAS12] = 50;
    GangInfo[gangid][gWeapons][GANGWEAPON_SNIPER] = 50;
