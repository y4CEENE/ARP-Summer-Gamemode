...
CMD:factionhelp(playerid, params[])
{
	...
	switch(FactionInfo[PlayerData[playerid][pFaction]][fType])
	{
		...
		{
		    SendClientMessage(playerid, COLOR_GREY, "** FACTION ** /gov /airstrike /getbomb /passport /plantcarbomb /pickupcarbomb /detonatecar");
		    SendClientMessage(playerid, COLOR_GREY, "** FACTION ** /hfind, /checkbomb, /d");
		}
	}
...
CMD:createfaction(playerid, params[])
{
	...
	if(sscanf(params, "s[12]s[48]", type, name))
	{
		...
		SendClientMessage(playerid, COLOR_SYNTAX, "List of types: Police, Medic, News, Government, Hitman, Federal, Army, Terrorist");
		return 1;
	}
...
CMD:hfind(playerid, params[])
{
    ...
    if(GetPlayerFaction(playerid) != FACTION_HITMAN && GetPlayerFaction(playerid) != FACTION_FEDERAL && GetPlayerFaction(playerid) != FACTION_TERRORIST)
	{
