...
CMD:g(playerid, params[])
{
	...
	if(!enabledGlobal && PlayerData[playerid][pAdmin] < JUNIOR_ADMIN)
	{
	...
	foreach(new i : Player)
	{
	  	if(!PlayerData[i][pToggleGlobal] && PlayerData[i][pLogged])
	  	{
	    	...
	    	{
				SendClientMessageEx(i, COLOR_GLOBAL, "(( %s: %.*s...))", GetRPName(playerid), MAX_SPLIT_LENGTH, text);
	        	SendClientMessageEx(i, COLOR_GLOBAL, "(( %s: ...%s ))", GetRPName(playerid), text[MAX_SPLIT_LENGTH]);
	   		}
	   		else
	   		{
	       		SendClientMessageEx(i, COLOR_GLOBAL, "(( %s: %s ))", GetRPName(playerid), text);
	   		}
	  	}
 	}
