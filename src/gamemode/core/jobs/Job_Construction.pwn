...
CMD:makeciment(playerid, params[])
{
    ...
    SetPlayerCheckpoint(playerid, MCCycle[MCIndex[playerid]][ccX], MCCycle[MCIndex[playerid]][ccY], MCCycle[MCIndex[playerid]][ccZ], 5.0);
    PlayerData[playerid][pCP] = CHECKPOINT_MAKECIMENT;
    return 1;
}
...
{
	if(PlayerData[playerid][pCP] == CHECKPOINT_MAKECIMENT)
    {
        if(IsPlayerInAnyVehicle(playerid))
        {
            return SendClientMessage(playerid, COLOR_GREY, "You can't work while in vehicle.");
        ...
        if(MCIndex[playerid] + 1 == sizeof(MCCycle))
        {
            ...
            SendClientMessageEx(playerid, COLOR_AQUA, "You have earned {00AA00}$%i{33CCFF} on your paycheck for your work.", amount);
            ClearAnimations(playerid, 1);
