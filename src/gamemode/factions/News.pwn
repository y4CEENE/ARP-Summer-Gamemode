...
CMD:liveban(playerid, params[])
{
    ...
    {
        return SendClientMessageEx(playerid, COLOR_GREY, "You need to be at least rank 4+ to use this command.");
    }
    if (sscanf(params, "u", targetid))
    {
...
CMD:adwithdraw(playerid, params[])
{
    ...
    DBLog("log_faction", "%s (uid: %i) has withdrawn $%i from the advertisement vault, reason: %s", GetPlayerNameEx(playerid), PlayerData[playerid][pID], amount, reason);
    return 1;
}
...
CMD:addeposit(playerid, params[])
{
    ...
    DBLog("log_faction", "%s (uid: %i) has deposited $%i in the advertisement vault.", GetPlayerNameEx(playerid), PlayerData[playerid][pID], amount);
    return 1;
}
