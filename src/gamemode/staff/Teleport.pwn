...
static const interiorArray[][intEnum] =
{
...
CMD:gotoint(playerid, params[])
{
    ...
    if (isnull(list))
    {
        ...
        {
            if (i == 0)
                format(list, sizeof(list), "%s", interiorArray[i][intName]);
            else
...
CMD:sendto(playerid, params[])
{
    ...
    if (!IsAdmin(playerid, ADMIN_LVL_3) && PlayerData[playerid][pHelper] < 3)
    {
        return SendUnauthorized(playerid);
    }
    if (!IsAdminOnDuty(playerid))
    ...
    if (!IsAdmin(playerid, ADMIN_LVL_3) && isnull(PlayerData[targetid][pHelpRequest]) && PlayerData[playerid][pAcceptedHelp] == 0)
    {
    ...
    else if (!strcmp(option, "player", true))
    {
        ...
        if (!IsPlayerLoggedIn(sendtargetid))
        {
...
CMD:goto(playerid, params[])
{
    ...
    if (!IsAdmin(playerid))
    {
        // Only helper no admin <Disabled for the moment>
        ...
        {
            return SendClientMessage(playerid, COLOR_GREY, "You can teleport only to newbie players (max level 3).");
        }
...
CMD:getcar(playerid, params[])
{
    ...
    if ((driverid = GetVehicleDriver(vehicleid)) != INVALID_PLAYER_ID && IsAdmin(playerid, GetAdminLvl(driverid) + 1))
    {
...
{
    new Float:x, Float:y, Float:z, interiorid, vwid;
    ...
    if (sscanf(params, "fffI(0)I(0)", x, y, z, interiorid, vwid))
    {
        return SendClientMessage(playerid, COLOR_SYNTAX, "USAGE: /gotocoords [x] [y] [z] [int (optional)] [vw (optional)]");
    }
