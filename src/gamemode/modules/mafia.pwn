...
CMD:gmats(playerid, params[])
{
    ...
    if (GetPlayerState(playerid) != PLAYER_STATE_ONFOOT)
        return SendClientMessage(playerid, COLOR_GREY, "You must be on foot in order to use this command.");
...
hook OnPlayerEnterCheckpoint(playerid)
{
    ...
    new earned = Mafia_Farming_Base + (random(4001) + 3000); // 3000-7000 + base

    // ✅ Add earned materials to player
    PlayerData[playerid][pMaterials] += earned;
    ...
    mysql_format(connectionID, queryBuffer, sizeof(queryBuffer),
        "UPDATE users SET materials = %i WHERE uid = %i",
        PlayerData[playerid][pMaterials], PlayerData[playerid][pID]);
