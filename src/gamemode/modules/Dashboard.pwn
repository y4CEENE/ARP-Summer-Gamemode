...
hook OnPlayerInit(playerid)
{
    ...
    DashboardUI[playerid][34] = CreatePlayerTextDraw(playerid, 62.666610, 153.496276,
        "Marb7e_Bik_fi_Arabica_Roleplay~n~"\
        "________2_Years_Of_Roleplay~n~"\
        "_Server_Rules:_arabicarp.com/server-offenses/~n~"\
        "_Server_Discord:_discord.gg/arabicarp~n~"\
        "_We_Need_To_Your_Support_With_Donation~n~"\
        "_|>[E-Dinar&Paypal&RunPay]~n~"\
        "_|>arabicarp.com/donation/~n~"\
        "_Choose_Anything_And_Contact_Us_In_Our_Discord~n~"\
        "_Arabica_Media_Channels:~n~"\
        "_|->campsite.bio/arabica~n~");
        /*Text*length**********************************/
...
DisplayDashboard(playerid)
{
    ...
    format(title,sizeof(title),"%s~n~%d years~n~%s~n~%s",
        ...
        (PlayerData[playerid][pMarriedTo] == -1)?("Single"):("Married")
        );
