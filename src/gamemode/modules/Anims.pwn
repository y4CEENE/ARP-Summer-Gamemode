...
/// @date      Created at 2021-05-09 15:39:39 +0100
...
CanPlayerUseAnim(playerid)
{
    if (GetPlayerSpecialAction(playerid) == SPECIAL_ACTION_CUFFED
     ...
     || PlayerData[playerid][pTazedTime]
...
CMD:testanim(playerid, params[])
{
    ...
    {
        new lib[32], name[32];
        if (sscanf(params, "s[32]s[32]", lib, name))
        {
            return SendClientMessage(playerid, COLOR_GREY, "Usage: /testanim [lib] [name]");
        }
        if (!IsAblePedAnimation(playerid))
            return 1;
        if (GetPlayerAnimationIndex(playerid) != 0)
            ClearAnimations(playerid);
        ApplyAnimation(playerid, lib, name, 4.1, 0, 0, 0, 1, 0);
        return 1;
    }
    return 0;
}

CMD:testhandsup(playerid, params[])
{
    if (!IsAblePedAnimation(playerid))
        return 1;
    if (GetPlayerAnimationIndex(playerid) != 0)
    ...
    switch (strval(params))
    {
        case 1: PlayAnim(playerid, "ped", "handsup", 4.1, 0, 0, 0, 1, 0);
        case 2: PlayAnim(playerid, "rob_bank", "shp_handsup_scr", 4.1, 0, 0, 0, 1, 0);
        case 3: PlayAnim(playerid, "shop", "shp_handsup_scr", 4.1, 0, 0, 0, 1, 0);
        case 4: PlayAnim(playerid, "shop", "shp_rob_handsup", 4.1, 0, 0, 0, 1, 0);
        case 5: PlayAnim(playerid, "shop", "shp_rob_react", 4.1, 0, 0, 0, 1, 0);
        case 6: PlayAnim(playerid, "samp", "handsupback", 4.1, 0, 0, 0, 1, 0);
        case 7: PlayAnim(playerid, "samp", "handsnair", 4.1, 0, 0, 0, 1, 0);
        case 8: PlayAnim(playerid, "samp", "handsuponknee", 4.1, 0, 0, 0, 1, 0);
        case 9: PlayAnim(playerid, "samp", "handsuponknee_idle", 4.1, 0, 0, 0, 1, 0);
        case 10: PlayAnim(playerid, "samp", "handsuponknee_stand", 4.1, 0, 0, 0, 1, 0);
        case 11: PlayAnim(playerid, "samp", "cs_handsuprelax", 4.1, 0, 0, 0, 1, 0);
        case 12: PlayAnim(playerid, "samp", "handsupdairboi", 4.1, 0, 0, 0, 1, 0);
        default: SendClientMessage(playerid, COLOR_SYNTAX, "USAGE: /testhandsup [1-12]");
    }
    return 1;
}

stock PlayChatAnimation(playerid, textLength)
{
    if (GetPlayerAnimationIndex(playerid) > 0)
    {
        return 1;
    ...
    if      (textLength <= 16)  SetTimerEx("EndChatAnim",2000, 0, "i", playerid);
    else if (textLength <= 32)  SetTimerEx("EndChatAnim",3500, 0, "i", playerid);
    else if (textLength <= 64)  SetTimerEx("EndChatAnim",4000, 0, "i", playerid);
    else if (textLength <= 96)  SetTimerEx("EndChatAnim",4500, 0, "i", playerid);
    else if (textLength <= 128) SetTimerEx("EndChatAnim",5000, 0, "i", playerid);
    else                        SetTimerEx("EndChatAnim",5500, 0, "i", playerid);
    return 1;
}

publish EndChatAnim(playerid)
{
