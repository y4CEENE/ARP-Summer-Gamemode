...
hook OnTwoPlayersHeartBeat(playerid, targetid)
{
    ...
    else if(PlayerData[playerid][pJailType] == 2) 
    {
        color = 0xAD7A2100;
        display_on_minimap = false;
    }
    else if(PlayerHasJob(playerid, JOB_TAXIDRIVER) && PlayerData[playerid][pTaxiFare] > 0) 
    {
        color = 0xF5DEB300;
        display_on_minimap = true;
    }
    ...
    {
        color = FactionInfo[PlayerData[playerid][pFaction]][fColor];
        display_on_minimap = true;
    }
    else
    {
