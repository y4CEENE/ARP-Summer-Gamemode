...
CMD:claimgift(playerid, params[])
{
    ...
    mysql_format(connectionID, queryBuffer, sizeof(queryBuffer), "UPDATE users SET minigame_claim = %i, minigame_count = %i WHERE uid = %i", PlayerMiniGameClaim[playerid], PlayerMiniGameCount[playerid], PlayerData[playerid][pID]);
...
{
    Create3DTextLabel("{FFFF00}Maze Minigame\n/joinminigame", -1, 1882.27, -1156.53, 24.78, 20.0, 0, 0);
    Create3DTextLabel("{FFFF00}You have reached the end of Maze Minigame\n\nYou can /claimgift or /exitminigame at anytime\n\nYou may exit with /exitminigame after doing so.", -1, -27.7987,1626.9500,1026.5034, 7.0, 0, 0);
    
    Create3DTextLabel("{FFFF00}Next area\nPress 'Y' to go up", -1, 62.7540,1625.8625,1057.2513, 5.0, 0, 0);
    Create3DTextLabel("{FFFF00}Next area\nPress 'Y' to go up", -1, 62.4150,1629.4210,1057.2513, 5.0, 0, 0);
    Create3DTextLabel("{FFFF00}Next area\nPress 'Y' to go up", -1, 26.7361,1631.9501,1013.6207, 5.0, 0, 0);
    
    Create3DTextLabel("{FFFF00}Fell down?\nPress 'Y' to go up", -1, 11.0897,1630.8412,1005.1062, 5.0, 0, 0);
    Create3DTextLabel("{FFFF00}Fell down?\nPress 'Y' to go up", -1, 0.5273,1631.2433,1005.1062, 5.0, 0, 0);
    Create3DTextLabel("{FFFF00}Fell down?\nPress 'Y' to go up", -1, -10.3871,1631.3064,1005.1062, 5.0, 0, 0);
    Create3DTextLabel("{FFFF00}Fell down?\nPress 'Y' to go up", -1, -20.5898,1626.3257,1005.1062, 5.0, 0, 0);
    Create3DTextLabel("{FFFF00}Fell down?\nPress 'Y' to go up", -1, -19.3437,1637.7467,1005.1062, 5.0, 0, 0);
    Create3DTextLabel("{FFFF00}Fell down?\nPress 'Y' to go up", -1, -28.2381,1632.5438,1005.1062, 5.0, 0, 0);    
    return 1;
}
...
hook OnPlayerHeartBeat(playerid)
{
    ...
    {
        if(IsPlayerInRangeOfPoint(playerid, 40.0, 1893.38135, -1176.850708, 24.460311) && IsPlayerSpawned(playerid) && !IsPlayerInAnyVehicle(playerid))
        {
...
{
    PlayerMiniGameClaim[playerid] = cache_get_field_content_int(row, "minigame_claim");
    PlayerMiniGameCount[playerid] = cache_get_field_content_int(row, "minigame_count");
    return 1;
}
...
{
    PlayerMiniGameClaim[playerid] = 0;
    mysql_format(connectionID, queryBuffer, sizeof(queryBuffer), "UPDATE users SET minigame_claim = %i WHERE uid = %i", PlayerMiniGameClaim[playerid], PlayerData[playerid][pID]);
	mysql_tquery(connectionID, queryBuffer);
    return 1;
}
