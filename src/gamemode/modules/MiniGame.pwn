...
{
    if (!IsPlayerInRangeOfPoint(playerid, 5.0, 957.937744, -1985.063598, 6.002842))
    {
...
CMD:claimgift(playerid, params[])
{
    ...
    format("UPDATE users SET minigame_claim = %i, minigame_count = %i WHERE uid = %i", PlayerMiniGameClaim[playerid], PlayerMiniGameCount[playerid], PlayerData[playerid][pID]);
...
{
    CreateDynamic3DTextLabel("{FFFF00}Maze Minigame\n/joinminigame", -1, 957.937744, -1985.063598, 6.002842, 20.0);
    CreateDynamic3DTextLabel("{FFFF00}Maze Fallout\n/joinfallout", -1, 945.911132, -1892.398071, 10.184990, 20.0);
...
hook OnPlayerHeartBeat(playerid)
{
    ...
    {
        if (IsPlayerInRangeOfPoint(playerid, 70.0, 930.802185, -1923.883911, 7.077081) && IsPlayerLoggedIn(playerid) && !IsPlayerInAnyVehicle(playerid))
        {
            PlayAudioStreamForPlayer(playerid, "http://f.top4top.io/m_3551o961d1.mp3");
            SetPVarInt(playerid, "MiniGameSong", 1);
        }
    }
    else if ( GetPVarInt(playerid, "MiniGameSong") == 1)
    {
        if (!IsPlayerInRangeOfPoint(playerid, 70.0, 930.8020, -1923.8463, 7.0770) || !IsPlayerLoggedIn(playerid) || IsPlayerInAnyVehicle(playerid))
        {
...
hook OnPlayerPayDay(playerid, cash)
{
    ...
    format("UPDATE users SET minigame_claim = %i WHERE uid = %i", PlayerMiniGameClaim[playerid], PlayerData[playerid][pID]);
    return 1;
}
