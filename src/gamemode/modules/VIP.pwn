...
/// @date      Created at 2022-01-23 17:29:49 +0100
...
DB:ListVip(playerid)
{
    ...
    SendClientMessage(playerid, COLOR_VIP, "---- VIP List ----");

    for (new i = 0; i < rows; i ++)
    {
        GetDBStringField(i, "username", username);
        GetDBStringField(i, "lastlogin", lastlogin);
        viplevel = GetDBIntField(i, "vippackage");
        viptime = GetDBIntField(i, "viptime");
        SendClientMessageEx(playerid, COLOR_GREY2, "[%i] {D909D9}%s VIP{C8C8C8} %s - Last Seen: %s - Expires: %s", viplevel, GetVIPRank(viplevel), username, lastlogin, TimestampToString(viptime, 4));
    }
}

DB:OnSyncRemoveVip()
{
    printf("[Script] %i vips removed.", GetDBNumRows());
}
...
CMD:changegender(playerid, params[])
{
    ...
    {
        return SendClientMessage(playerid, COLOR_GREY, "I'm afraid we don't do that here.");
    }

    SendClientMessageEx(playerid, COLOR_WHITE, "You are now %s.", GetPlayerGenderStr(playerid));
    DBQuery("UPDATE "#TABLE_USERS" SET gender = %d WHERE uid = %i",
...
CMD:changeage(playerid, params[])
{
    ...
    {
        return SendClientMessageEx(playerid, COLOR_SYNTAX, "USAGE: /changeage [%i <= value <= %i ]", MIN_PLAYER_AGE, MAX_PLAYER_AGE);
    }
    ...
    DBQuery("UPDATE "#TABLE_USERS" SET age = %i WHERE uid = %i", value, PlayerData[playerid][pID]);
...
CMD:locatelounge(playerid, params[])
{
    ...
    if (GetPlayerDistanceFromPoint(playerid, 2114.1025, -1743.0543, 13.5547) < GetPlayerDistanceFromPoint(playerid, -1549.5186,  1171.9185,  7.1875))
    {
        SetActiveCheckpoint(playerid, CHECKPOINT_MISC, 2114.1025, -1743.0543, 13.5547, 2.5);
        SendClientMessage(playerid, COLOR_GREEN, "Waypoint set to LS VIP Lounge");
    }
    else
    {
        SetActiveCheckpoint(playerid, CHECKPOINT_MISC, -1549.5186,  1171.9185,  7.1875, 2.5);
...
CMD:changeplates(playerid, params[])
{
    ...
    DBQuery("UPDATE vehicles SET plate = '%e' WHERE id = %i", VehicleInfo[vehicleid][vPlate], VehicleInfo[vehicleid][vID]);
...
CMD:ovips(playerid, params[])
{
    ...
    DBQueryWithCallback("SELECT username, lastlogin, vippackage, viptime FROM "#TABLE_USERS" WHERE vippackage > 0 ORDER BY vippackage DESC", "ListVip", "i", playerid);
    return 1;
}
...
CMD:viphelp(playerid, params[])
{
    ...
    SendClientMessage(playerid, COLOR_GREY, "** SILVER VIP ** /vipinfo /(v)ip /locatelounge /viplocker /vipcolor /vipinvite /vipnumber /vipmusic");

    if (PlayerData[playerid][pDonator] >= 2)
    {
        SendClientMessage(playerid, COLOR_GREY, "** GOLD VIP ** /changeage /changegender");
    }
    if (PlayerData[playerid][pDonator] == 3)
    {
        SendClientMessage(playerid, COLOR_GREY, "** LEGENDARY VIP ** /repair /nos /hyd /changeplates");
    }
    return 1;
}
