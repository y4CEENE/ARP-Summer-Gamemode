...
AddReportToQueue(playerid, text[])
{
    for (new x = 0; x < MAX_REPORTS; x ++)
    {
        if (!ReportInfo[x][rExists])
        {
            ...
            {
                if (IsAdmin(i) && !PlayerData[i][pToggleReports])
                {
...
CMD:ar(playerid, params[])
{
    ...
    DBQuery("UPDATE "#TABLE_USERS" SET reports = %i WHERE uid = %i", PlayerData[playerid][pReports], PlayerData[playerid][pID]);
...
CMD:nor(playerid, params[])
{
    ...
    SendClientMessageEx(ReportInfo[reportid][rReporter], COLOR_LIGHTRED,
        "%s has trashed your report as your request for a revive is invalid. (/phone > call > %d)",
        GetRPName(playerid),
...
CMD:report(playerid, params[])
{
    ...
    if (IsReportMuted(playerid))
    {
        return SendClientMessageEx(playerid, COLOR_GREY, "You are muted from submitting reports. /unmute to unmute yourself.");
    }
...
Dialog:DlgReportType(playerid, response, listitem, inputtext[])
{
    ...
    if (response && listitem>=0)
    {
        ...
        switch (listitem)
        {
            case 0: Dialog_Show(playerid, DlgReportSubject, DIALOG_STYLE_LIST, title, "RVDM\nTK\nDM\nRK\nSK\nPG\nMG\nLTA\nRT\nNRCR\n0 VOL\nCOP BAITING\nNo RP behavior\nRandom shooting\nRandom pushing\nScam\nRRV", "Select", "Cancel");
...
Dialog:DlgReportDetails(playerid, response, listitem, inputtext[])
{
    if (response)
    {
        ...
        else
        {
            ...
            new details[128];
            format(details, sizeof(details), "[%s%s::%s] %s", sf, PlayerReportType[playerid], PlayerReportSubject[playerid], inputtext);
