...
static PizzaPayment_Base = 150;
static PizzaPayment_TipPerLevel = 50;
static PizzaPayment_TipPercent = 5;
...
hook OnLoadGameMode(timestamp)
{
    ...
    new Node:job;
    new Node:pizzaman;
    if (!GetServerConfig("jobs", job) && !JSON_GetObject(job, "pizzaman", pizzaman))
    {
        JSON_GetInt(pizzaman, "payment_base",          PizzaPayment_Base);
        JSON_GetInt(pizzaman, "payment_tip_per_level", PizzaPayment_TipPerLevel);
        JSON_GetInt(pizzaman, "payment_tip_percent",   PizzaPayment_TipPercent);
    }
    return 1;
}
...
hook OnPlayerInit(playerid)
{
    ...
    PlayerLastPizza[playerid] = 0;
    PlayerPizzaTime[playerid] = 0;
    PlayerPizzaCooldown[playerid] = 0;
    return 1;
}
...
{
    if (type != CHECKPOINT_PIZZA)
        return 1;

    new string[32];
    new amount = PizzaPayment_Base + GetJobLevel(playerid, JOB_PIZZAMAN) * PizzaPayment_TipPerLevel + random(PizzaPayment_TipPerLevel);
    new tip = percent(amount, PizzaPayment_TipPercent);

    if (gettime() - PlayerLastPizza[playerid] < 15 && !IsAdmin(playerid, ADMIN_LVL_3) && !PlayerData[playerid][pKicked])
    {
...
{
    PlayerPizzaCooldown[playerid] = GetDBIntField(row, "pizzacooldown");
    return 1;
}
...
CMD:getpizza(playerid, params[])
{
    ...
    SetActiveCheckpoint(playerid, CHECKPOINT_PIZZA, HouseInfo[houseid][hPosX], HouseInfo[houseid][hPosY], HouseInfo[houseid][hPosZ], 2.0);
