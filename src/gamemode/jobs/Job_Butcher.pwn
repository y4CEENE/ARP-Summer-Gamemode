/// @file      Job_Butcher.pwn
/// @author    BOURAOUI Al-Moez L.A
/// @date      Created at 2021-10-13 22:26:13 +0100
/// @copyright Copyright (c) 2022

#include <YSI\y_hooks>

static ButcherSkinId  = 168;
static MinButcherRate = 60;
static MaxButcherRate = 120;
...
hook OnLoadGameMode(timestamp)
{
    new Node:job;
    new Node:config;
    if (!GetServerConfig("jobs", job) && !JSON_GetObject(job, "butcher", config))
    {
        JSON_GetInt(config, "skin_id",     ButcherSkinId);
        JSON_GetInt(config, "min_payment", MinButcherRate);
        JSON_GetInt(config, "max_payment", MaxButcherRate);
    }
}
...
CMD:butcher(playerid)
{
    ...
    else if (!IsPlayerButcher[playerid])
    {
        ...
        SetPlayerSkin(playerid, ButcherSkinId);
...
hook OnPlayerReachCheckpoint(playerid, type, flag)
{
    ...
    if (sizeof(ButcherData) <= ButcherTask[playerid] < 0)
    {
        return 1;
    }

    if (GetPlayerSkin(playerid) != ButcherSkinId)
    {
        ButcherTask[playerid] = 0;
        IsPlayerButcher[playerid] = false;
        SendClientMessage(playerid, COLOR_RED, "You need to wear the butcher clothes!");
        return 1;
    }
...
publish GetMeatTimer(playerid)
{
    ...
    if (!IsPlayerNearPoint(playerid,  2.0, x, y, z))
    {
        return SendClientMessageEx(playerid, COLOR_RED, "You lost your meat as you moved.");
    }
...
SetButcherTaskCheckpoint(playerid)
{
    ...
    SetActiveCheckpoint(playerid, CHECKPOINT_BUTCHER, x, y, z, 2.0);
