...
static ScubaDiver_JellyfishPrice = 40;
static ScubaDiver_MaxInventory = 24;
...
hook OnLoadGameMode(timestamp)
{
    new Node:job;
    new Node:config;
    if (!GetServerConfig("jobs", job) && !JSON_GetObject(job, "scuba_diver", config))
    {
        JSON_GetInt(config, "jellyfish_price", ScubaDiver_JellyfishPrice);
        JSON_GetInt(config, "max_inventory",   ScubaDiver_MaxInventory);
    }
    ...
    for (new i=0;i<sizeof(JellyFish);i++)
    {
        if (random(2))
            ...
            JellyFish[i][jfPickupID] = CreateDynamicPickup(1603, 2, JellyFish[i][jfX], JellyFish[i][jfY], JellyFish[i][jfZ] + 1.0);
    }
    return 1;
}
...
PickUpJellyFish(playerid)
{
    ...
    new price = weight * ScubaDiver_JellyfishPrice;
    if (weight + ScubaDiverWeight[playerid] > ScubaDiver_MaxInventory)
    {
...
CMD:enddiving(playerid, params[])
{
    ...
    {
        new price = ScubaDiverWeight[playerid] * ScubaDiver_JellyfishPrice;
